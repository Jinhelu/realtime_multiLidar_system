    ...
    while(true){
        ...
        if(bufret == RslidarInput::INPUT_MSOP){
            ...
            if (ret == Rslidar::RS_Decode_ok) {
                if(timeFalg) {
                    timestampFirst=timestamp; timeFalg=false;
                    long long IMUPCtime;
                ...
                if( !IMUdata_Q.writeLidarVisit((int)receiver.size(),PCTime_u,timestamp) ){
                    cout << "Write Lidar Visit in IMU Record file failed!" << endl;
                }
            ...
            else if(ret== Rslidar::RS_Frame_Split){//一帧结束，新一帧开始给予
